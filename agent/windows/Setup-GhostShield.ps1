...
if (-not (Get-Command node.exe -ErrorAction SilentlyContinue)) {
  throw "Node.js 22 or newer is required. Install Node.js, then run this setup again."
}
...
Invoke-CheckedCommand -FilePath "npm.cmd" -ArgumentList @("install", "--prefix", $agentRoot) -Description "Installing GhostShield agent dependencies"
Invoke-CheckedCommand -FilePath "npm.cmd" -ArgumentList @("run", "build:dashboard", "--prefix", $agentRoot) -Description "Building the GhostShield command center"
...
Write-Host "Next: run Install-GhostShieldStartup.ps1 from an Administrator PowerShell window."
